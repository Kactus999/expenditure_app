...
import com.example.expenditureapp.databinding.ActivityDetailWithdrawBinding
...
private lateinit var binding: ActivityDetailWithdrawBinding
class DetailWithdrawActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupDetailWithdrawLV()

    }

    private fun setupDetailWithdrawLV()
    {
        helper = UserDBHelper(applicationContext)
        db = helper.readableDatabase
        val rsTrans = db.rawQuery("SELECT * FROM TRANSACTIONS WHERE WALLET LIKE '%${walletName}' and TYPE LIKE '%${withdraw}' ORDER BY date(TRANS_DATE) DESC  ",null)
        ...
        if (rsTrans.moveToLast())
        {
            ...
            binding.lvDetailWithdraw.adapter = adapter
        }

        binding.lvDetailWithdraw.setOnItemLongClickListener { parent, view, position, id ->
        ...
        binding.lvDetailWithdraw.setOnItemClickListener { parent, view, position, id ->
            if (rsTrans.moveToPosition(position))
            {
    ...
    private fun showDeleteDialog(c : Cursor) {
        ...
        dialog.apply {
            ...
            setPositiveButton("Yes"){ dialogInterface: DialogInterface, i: Int ->
                ...
                val hisWitAmount = c.getString(1).toString().toDouble()
